...
## n.b. need devel version of lme4
...
```{r pbkrtest,warning=FALSE,message=FALSE}
library(pbkrtest)
t1 <- system.time(suppressMessages(
  PBmodcomp(lmer3,lmer4,nsim=500)
  ))
```
...
In many cases you will want to set up a new data frame to do prediction rather than just using the covariates from the original data (e.g. if the data are sampled irregularly, or sparsely), and use the `newdata` argument of `predict`.  The `expand.grid` function is handy in this context too.

Unfortunately computing confidence intervals for the predictions is a little tricky: there is some code in the [GLMM FAQ](https://bbolker.github.io/mixedmodels-misc/glmmFAQ.html) for this, but it may be easier to use the `effects`, `emmeans`, or `sjPlot` packages.

```{r emplot}
emdata <- as.data.frame(emmeans(lmer2,~mnth+roostsitu))
(g0
  + geom_line(data=emdata,aes(y=emmean,group=1),colour="red",size=2)
  + geom_ribbon(data=emdata,aes(y=emmean,
                                group=1,
